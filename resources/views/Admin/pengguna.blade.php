    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        <header class="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 flex-shrink-0">
            <div class="relative w-96">
                ...
                <input type="text" id="searchInput" placeholder="Cari NIP, NIM, atau Nama..." class="w-full bg-gray-50 border border-gray-200 rounded-full py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#004133]/20">
            </div>
    ...
    <script>
        ...
        document.getElementById('searchInput').addEventListener('keyup', function() {
            ...
            for (let i = 0; i < rows.length; i++) {
                let row = rows[i];
                if (row.getElementsByTagName('td').length > 1) { // Abaikan baris kosong (colspan)
